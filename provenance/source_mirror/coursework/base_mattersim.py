...
out_dir = "~/coursework/mattersim_eval"
...
# глушим спам: показывать варнинг "No PBC detected" максимум один раз (остальные уйдут в warn_log)
...
def eval_mattersim(cfgs, device="cuda", cutoff=5.0):
    calc = MatterSimCalculator(device=device, cutoff=cutoff)  # создаём один раз
...
with open(txt_out, "w") as f:
    ...
    train_res = eval_mattersim(NbMo_train_cfgs_from_DFT, device=device, cutoff=5.0)
    results["TRAIN"] = train_res
    write_report(f, "TRAIN", train_res)

    if NbMo_valid_cfgs_from_DFT is not None:
        valid_res = eval_mattersim(NbMo_valid_cfgs_from_DFT, device=device, cutoff=5.0)
        results["VALID"] = valid_res
        write_report(f, "VALID", valid_res)

    if NbMo_test_cfgs_from_DFT is not None:
        test_res = eval_mattersim(NbMo_test_cfgs_from_DFT, device=device, cutoff=5.0)
